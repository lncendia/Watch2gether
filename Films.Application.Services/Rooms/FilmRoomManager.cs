...
public class FilmRoomManager(IUnitOfWork unitOfWork, IFilmRoomMapper mapper, IMemoryCache memoryCache)
    : IFilmRoomManager
{
    ...
    {
        if (!memoryCache.TryGetValue(id, out FilmRoom? room))
        {
    ...
    private async Task<(Guid roomId, int viewerId)> AddAsync(FilmRoom room)
    {
        ...
        await unitOfWork.SaveChangesAsync();
        memoryCache.Set(room.Id, room, new MemoryCacheEntryOptions().SetAbsoluteExpiration(TimeSpan.FromMinutes(5)));

...
internal class FilmModelMapper(ApplicationDbContext context) : IModelMapperUnit<FilmModel, Film>
{
    ...
    private async Task ProcessCdnAsync(Film entity, FilmModel film)
    {
        ...
        foreach (var cdn in entity.CdnList)
        {
            ...
            cdnModel.Url = cdn.Url;
            cdnModel.Quality = cdn.Quality;
            cdnModel.Type = cdn.Type;

            var newVoices = cdn.Voices.Where(x =>
                    cdnModel.Voices.All(m => !string.Equals(m.Name, x, StringComparison.CurrentCultureIgnoreCase)))
                .ToList();
            var removeVoices = cdnModel.Voices.Where(x =>
                cdn.Voices.All(m => !string.Equals(m, x.Name, StringComparison.CurrentCultureIgnoreCase)));
            var databaseVoices = await context.Voices.Where(x =>
                    newVoices.Select(s => s.ToUpper())
                        .Any(v => v.Equals(x.Name, StringComparison.CurrentCultureIgnoreCase)))
                .ToListAsync();
            databaseVoices =
                databaseVoices.Concat(_voices.Where(x =>
                    newVoices.Any(v => string.Equals(v, x.Name, StringComparison.CurrentCultureIgnoreCase)))).ToList();
            var notDatabaseVoices = newVoices.Where(x =>
                    databaseVoices.All(v => !string.Equals(v.Name, x, StringComparison.CurrentCultureIgnoreCase)))
    ...
    {
        var newCountries =
            entity.Countries.Where(x =>
                film.Countries.All(m => !string.Equals(m.Name, x, StringComparison.CurrentCultureIgnoreCase))).ToList();
        var databaseCountries = await context.Countries
            .Where(x => newCountries.Select(s => s.ToUpper())
                .Any(c => c.Equals(x.Name, StringComparison.CurrentCultureIgnoreCase)))
            .ToListAsync();
        databaseCountries =
            databaseCountries.Concat(_countries.Where(x =>
                    newCountries.Any(c => string.Equals(c, x.Name, StringComparison.CurrentCultureIgnoreCase))))
                .ToList();
        var notDatabaseCountries = newCountries.Where(x =>
                databaseCountries.All(c => !string.Equals(c.Name, x, StringComparison.CurrentCultureIgnoreCase)))
    ...
    {
        var newActors =
            entity.Actors.Where(x =>
                    film.Actors.All(m => !m.Person.Name.Equals(x.Name, StringComparison.CurrentCultureIgnoreCase)))
                .ToList();
        var databaseActors = await context.Persons
            .Where(x => newActors.Select(s => s.Name.ToUpper())
                .Any(p => p.Equals(x.Name, StringComparison.CurrentCultureIgnoreCase)))
            .ToListAsync();
        databaseActors =
            databaseActors.Concat(_persons.Where(x =>
                    newActors.Any(p => p.Name.Equals(x.Name, StringComparison.CurrentCultureIgnoreCase))))
                .ToList();
        var notDatabaseActors = newActors.Where(x =>
                databaseActors.All(p => !p.Name.Equals(x.Name, StringComparison.CurrentCultureIgnoreCase)))
    ...
    {
        var newGenres =
            entity.Genres.Where(x =>
                film.Genres.All(m => !string.Equals(m.Name, x, StringComparison.CurrentCultureIgnoreCase))).ToList();
        var databaseGenres = await context.Genres
            .Where(x => newGenres.Select(s => s.ToUpper())
                .Any(g => g.Equals(x.Name, StringComparison.CurrentCultureIgnoreCase)))
            .ToListAsync();
        databaseGenres =
            databaseGenres.Concat(_genres.Where(x =>
                    newGenres.Any(g => string.Equals(g, x.Name, StringComparison.CurrentCultureIgnoreCase))))
                .ToList();
        var notDatabaseGenres = newGenres.Where(x =>
                databaseGenres.All(g => !string.Equals(g.Name, x, StringComparison.CurrentCultureIgnoreCase)))
    ...
    {
        var newDirectors =
            entity.Directors.Where(x =>
                film.Directors.All(m => !string.Equals(m.Name, x, StringComparison.CurrentCultureIgnoreCase))).ToList();
        var databaseDirectors = await context.Persons
            .Where(x => newDirectors.Select(s => s.ToUpper())
                .Any(p => p.Equals(x.Name, StringComparison.CurrentCultureIgnoreCase)))
            .ToListAsync();
        databaseDirectors =
            databaseDirectors.Concat(_persons.Where(x =>
                    newDirectors.Any(p => string.Equals(p, x.Name, StringComparison.CurrentCultureIgnoreCase))))
                .ToList();
        var notDatabaseDirectors = newDirectors.Where(x =>
                databaseDirectors.All(p => !string.Equals(p.Name, x, StringComparison.CurrentCultureIgnoreCase)))
    ...
    private async Task ProcessScreenwritersAsync(Film entity, FilmModel film)
    {
        var newScreenwriters =
            entity.Screenwriters.Where(x =>
                    film.ScreenWriters.All(m => !string.Equals(m.Name, x, StringComparison.CurrentCultureIgnoreCase)))
                .ToList();
        var databaseScreenwriters = await context.Persons
            .Where(x => newScreenwriters.Select(s => s.ToUpper())
                .Any(p => p.Equals(x.Name, StringComparison.CurrentCultureIgnoreCase)))
            .ToListAsync();
        databaseScreenwriters =
            databaseScreenwriters
                .Concat(_persons.Where(x =>
                    newScreenwriters.Any(p => string.Equals(p, x.Name, StringComparison.CurrentCultureIgnoreCase))))
                .ToList();
        var notDatabaseScreenwriters = newScreenwriters
            .Where(x => databaseScreenwriters.All(p =>
                !string.Equals(p.Name, x, StringComparison.CurrentCultureIgnoreCase)))
            .Select(p => new PersonModel { Name = p }).ToList();

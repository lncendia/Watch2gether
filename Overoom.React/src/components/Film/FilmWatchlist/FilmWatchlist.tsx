import styles from "./FilmWatchlist.module.css"
import {OverlayTrigger, Tooltip} from "react-bootstrap";

const renderTooltip = () => (
    <Tooltip>Смотреть позже</Tooltip>
);

const renderSvg = (inWatchlist: boolean, onWatchlistToggle: () => void) => {
    if (inWatchlist) return (
        <svg className={`${styles.button} bi bi-clock-fill`} onClick={onWatchlistToggle}
             xmlns="http://www.w3.org/2000/svg" width="16" height="16"
             fill="currentColor" viewBox="0 0 16 16">
            <path
                d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM8 3.5a.5.5 0 0 0-1 0V9a.5.5 0 0 0 .252.434l3.5 2a.5.5 0 0 0 .496-.868L8 8.71V3.5z"/>
        </svg>
    )

    return (
        <svg className={`${styles.button} bi bi-clock`} onClick={onWatchlistToggle}
             xmlns="http://www.w3.org/2000/svg"
             width="16" height="16"
             fill="currentColor" viewBox="0 0 16 16">
            <path d="M8 3.5a.5.5 0 0 0-1 0V9a.5.5 0 0 0 .252.434l3.5 2a.5.5 0 0 0 .496-.868L8 8.71V3.5z"/>
            <path d="M8 16A8 8 0 1 0 8 0a8 8 0 0 0 0 16zm7-8A7 7 0 1 1 1 8a7 7 0 0 1 14 0z"/>
        </svg>
    )
}

const FilmWatchlist = ({inWatchlist, onWatchlistToggle}: { inWatchlist: boolean, onWatchlistToggle: () => void }) => {

    return (
        <OverlayTrigger
            placement="left"
            delay={{show: 250, hide: 400}}
            overlay={renderTooltip}
        >
            {renderSvg(inWatchlist, onWatchlistToggle)}
        </OverlayTrigger>
    )
};

export default FilmWatchlist;
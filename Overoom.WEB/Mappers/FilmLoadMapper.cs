...
{
    public FilmLoadDto Map(FilmLoadParameters parameters)
    {
        var uri = parameters.PosterUri == null ? null : new Uri(parameters.PosterUri);
        var cdnList = parameters.Cdns
            .Select(x => new CdnDto(x.Type, new Uri(x.Uri), x.Quality, x.Voices.Select(s => s.Name).ToList())).ToList();
        ...
        return new FilmLoadDto(parameters.Name, parameters.Description, parameters.ShortDescription,
            parameters.Rating, parameters.Year, parameters.Type, uri, parameters.Poster?.OpenReadStream(), genres,

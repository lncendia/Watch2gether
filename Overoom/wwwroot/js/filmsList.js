...
function ParseData(json) {
    ...
    data.films.forEach(el => {
        ...
        html += '</div><a class="filmLink" href="/Film/Film?id=' + el.id + '"><img src="img/Posters/' + el.posterFileName + '" class="card-img-top poster" alt="..."><div class="card-body"><h5 class="card-title">' + el.name + ' (' + el.year + ')' + '</h5><h6 class="card-subtitle mb-2 filmDescriptionList">' + el.description + '</h6><p class="card-text pb-5">' + el.genres + '</p></div></a><div class="card-footer filmCardBottom">' + el.type + '</div></div></div>';
        films.append(html);
    })
    return true;
}

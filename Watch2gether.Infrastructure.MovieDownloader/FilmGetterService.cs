...
using Watch2gether.Infrastructure.MovieDownloader.Converters;
using Watch2gether.Infrastructure.MovieDownloader.Enums;
using Watch2gether.Infrastructure.MovieDownloader.Models;

namespace Watch2gether.Infrastructure.MovieDownloader;
...
public class FilmGetterService : IFilmInfoGetterService
{
    ...
    {
        var films = result.Films.Select(x => new FilmInfoDto(x.KpId!, x.Title, x.DateTime.Year, x.FilmType));
        ...
        IReadOnlyCollection<ActorsData> actorsData)
    {
        ...
        return new FilmFullInfoDto(data.Title, filmFullData.Description, filmFullData.ShortDescription,
            rating, data.DateTime.Year, data.FilmType, new Uri("https:" + data.Url), filmFullData.Genres, actors,

...
public class YoutubeRoomRepository : IYoutubeRoomRepository
{
    ...
    {
        var room = new YoutubeRoom(model.Url, "someName", "someAvatar");
    ...
    {
        _context.Remove(_context.YoutubeRooms.Include(x => x.Messages).Include(x => x.Viewers)
    ...
    {
        var ids = entities.Select(room => room.Id);
        _context.RemoveRange(_context.YoutubeRooms.Include(x => x.Messages).Include(x => x.Viewers)
